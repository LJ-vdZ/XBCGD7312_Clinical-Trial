...
public class CameraFollow : MonoBehaviour
{
    ...
    void HandleCameraPosition()
    {
        ...
        if (castDist > 0.001f)
        {
            int hitCount = Physics.RaycastNonAlloc(pivot, dir.normalized, hitBuffer, castDist,~0, QueryTriggerInteraction.Ignore);
    ...
    //only walls/floors avoid clipping. other props ignore
    ...
    public static bool IsWallOrFloorObjectName(string objectName)
    {
        ...
        //exclude these props from colission detection
        if (ContainsIgnoreCase(objectName, "Light"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Ray"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Shelf"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Painting"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Prop"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Trigger"))
        {
            return false;
        }

        if (ContainsIgnoreCase(objectName, "Decal"))
        {
            return false;
        }

        //only floor and walls and ceiling have collision detection
        if (StartsWithIgnoreCase(objectName, "Wall"))
        {
            return true;
        }

        if (StartsWithIgnoreCase(objectName, "Floor"))
        {
            return true;
        }

        if (StartsWithIgnoreCase(objectName, "Ceiling"))
        {
            return true;
        }
    ...
    static bool StartsWithIgnoreCase(string value, string prefix) =>value.StartsWith(prefix, System.StringComparison.OrdinalIgnoreCase);

    static bool ContainsIgnoreCase(string value, string token) => value.IndexOf(token, System.StringComparison.OrdinalIgnoreCase) >= 0;
